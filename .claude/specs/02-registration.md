...
- `database/db.py` — add `create_user()` helper (removes the now-superseded `get_user_by_email()` from a prior pass)
- `templates/register.html` — wire up form action/method, add `confirm_password` field
- `templates/base.html` — add the shared `get_flashed_messages()` rendering block (flash messages needed to
  survive the redirect from `/register` to `/login`, so rendering only in `register.html` wasn't sufficient)
- `static/css/style.css` — add `.flash-message` / `.flash-success` / `.flash-error` styles
